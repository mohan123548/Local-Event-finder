...
async function bootstrapDataset() {
  ...
  if (!hasFrench) {
    try {
      console.log("Fetching data/french_festivals.json...");
      const response = await fetch('data/french_festivals.json');
      if (response.ok) {
        ...
        console.error("Failed to fetch data/french_festivals.json");
      }
    } catch (error) {
